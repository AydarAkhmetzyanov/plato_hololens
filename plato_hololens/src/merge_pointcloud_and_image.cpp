...
void callback(const ImageConstPtr& image_color_msg, const MyPointCloudPtr& pointcloud_msg,
              ...
              const plato_hololens::BoolStampedConstPtr& is_moving_msg,
              const ros::Publisher *marker_pub, const tf2_ros::Buffer *buff){
                ...
                try
                {
                        ...
                        for (auto box : bb_msg->bounding_boxes) {
                                ...
                                if(detected_ojects.size()==0){
                                        ...
                                        int id = 0;
                                        if(class_n.c_str()=="Person"){
                                                id = 1;
                                        }else if(class_n.c_str()=="Window"){
                                                id = 2;
                                        }
                                        new_obj.class_id = id;
                        ...
                        for(int i=0; i<detected_ojects.size(); i++){
                                ...
                                if(detected_ojects[i].class_id==1){
                                        marker.color.r = 0.0f;
                                        marker.color.g = 1.0f;
                                        marker.color.b = 0.0f;
                                }else if(detected_ojects[i].class_id==2){
                                        marker.color.r = 0.0f;
                                        marker.color.g = 0.0f;
                                        marker.color.b = 1.0f;
                                }else{
...
int main (int argc, char** argv)
{
        ...
        // synchronizer
        typedef sync_policies::ApproximateTime<Image, MyPointCloud, CameraInfo, darknet_ros_msgs::BoundingBoxes,plato_hololens::BoolStamped> MySyncPolicy;
        Synchronizer<MySyncPolicy> sync(MySyncPolicy(10), image_color_sub, pointcloud_sub, info_color_sub, bounding_boxes_sub,is_moving_sub);
        sync.registerCallback(boost::bind(&callback, _1, _2, _3, _4, _5, &marker_pub, &tfBuffer));
